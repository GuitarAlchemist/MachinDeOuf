...
impl ReservoirNetwork {
    pub fn new(size: usize, input_dim: usize, spectral_radius: f64, sparsity: f64, leak_rate: f64, rng: &mut impl Rng) -> Self {
        ...
        for w in weights.iter_mut() {
            if rng.random::<f64>() > sparsity { *w = rng.sample(dist); }
        }
        let max_val = weights.iter().copied().map(f64::abs).fold(0.0_f64, f64::max);
        if max_val > 0.0 { weights.mapv_inplace(|x| x * spectral_radius / max_val); }
        let input_weights = Array2::from_shape_fn((input_dim, size), |_| rng.sample(dist) * 0.1);
        Self { size, weights, input_weights, state: Array1::zeros(size), spectral_radius, sparsity, leak_rate, readout: None }
    }
    ...
    pub fn step(&mut self, input: &Array1<f64>) -> &Array1<f64> {
        ...
        self.state = (1.0 - self.leak_rate) * &self.state + self.leak_rate * pre_activation.mapv(f64::tanh);
        &self.state
    }

    pub fn state(&self) -> &Array1<f64> { &self.state }
...
mod tests {
    ...
    fn test_reservoir_state_bounded() {
        ...
        for _ in 0..100 { r.step(&input); }
        for &v in r.state().iter() { assert!(v.abs() <= 1.0, "Unbounded: {}", v); }
    }
