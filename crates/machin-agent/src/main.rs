...
fn main() {
    ...
    eprintln!("[machin-mcp] MCP server started, listening on stdin...");

    for line in stdin.lock().lines() {
        let line = match line {
            Ok(l) => l,
            Err(e) => {
                eprintln!("[machin-mcp] stdin read error: {}", e);
                break;
            }
        };
        ...
        eprintln!("[machin-mcp] method={}", request.method);
    ...
    eprintln!("[machin-mcp] Server shutting down.");
}
...
fn handle_tools_call(id: Value, params: Option<Value>, registry: &ToolRegistry) -> JsonRpcResponse {
    ...
    eprintln!("[machin-mcp] tools/call name={}", tool_name);
