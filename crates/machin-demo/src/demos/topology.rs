...
impl TopologyDemo {
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ui.heading("Topology / Persistent Homology (machin-topo)");
    ...
    fn persistence_ui(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.button("Compute Persistence").clicked() {
            ...
            let diagrams = machin_topo::pointcloud::persistence_from_points(
                &self.points,
                2,
                self.max_radius,
            );
            ...
            self.top_features =
                machin_topo::pointcloud::most_persistent_features(&diagrams, self.top_k);
        }
    ...
    fn betti_curve_ui(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.button("Compute Betti Curve").clicked() {
            self.generate_points();
            let curve = machin_topo::pointcloud::betti_curve(&self.points, 2, self.betti_steps);
    ...
    fn betti_at_radius_ui(&mut self, ui: &mut egui::Ui) {
        ...
        if ui.button("Compute Betti Numbers").clicked() {
            self.generate_points();
            self.betti_at_r =
                machin_topo::pointcloud::betti_at_radius(&self.points, 2, self.betti_radius);
        }
