...
impl FractalDemo {
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ui.heading("Fractals (machin-fractal)");
    ...
    fn gen_takagi(&mut self) {
        let n_points = 501;
        let curve = machin_fractal::takagi::takagi_series(n_points, self.takagi_terms);
    ...
    fn gen_ifs(&mut self) {
        ...
        let maps = match self.ifs_preset {
            IfsPreset::Sierpinski => machin_fractal::ifs::sierpinski_maps(),
            IfsPreset::BarnsleyFern => machin_fractal::ifs::barnsley_fern_maps(),
            IfsPreset::Koch => machin_fractal::ifs::koch_snowflake_maps(),
        };
        let mut rng = rand::rngs::StdRng::seed_from_u64(42);
        self.plot_points = machin_fractal::ifs::ifs_iterate(&maps, self.ifs_iterations, &mut rng);
    }

    fn gen_lsystem(&mut self) {
        let (lsys, angle) = match self.lsystem_preset {
            LSystemPreset::Dragon => (machin_fractal::lsystem::dragon_curve(), 90.0),
            LSystemPreset::Sierpinski => (machin_fractal::lsystem::sierpinski_arrowhead(), 60.0),
            LSystemPreset::Koch => (machin_fractal::lsystem::koch_curve(), 60.0),
        };
        let expanded = lsys.expand(self.lsystem_iterations);
        self.plot_points = machin_fractal::lsystem::interpret(&expanded, angle, 1.0);
    }

    fn gen_space_filling(&mut self) {
        self.plot_points = match self.space_preset {
            SpaceFillingPreset::Hilbert => machin_fractal::space_filling::hilbert_curve(self.space_order),
            SpaceFillingPreset::Peano => machin_fractal::space_filling::peano_curve(self.space_order),
        };
    }
}
