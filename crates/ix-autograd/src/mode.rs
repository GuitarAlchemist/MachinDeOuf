...
//!               upstream tools run in `Eager` and their outputs are frozen
//!               as constants at the boundary. No gradient flows into
//!               non-differentiable tools. Claude's addition.
//! - `VerifyFiniteDiff` — run the finite-difference verifier on all tools
//!               that report `supports_grad() == true`. Used by CI and
//!               integration tests to catch backward bugs early.
