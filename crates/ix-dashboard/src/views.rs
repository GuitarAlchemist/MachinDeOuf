...
pub fn render_beliefs(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let rows: Vec<Row> = app.data.beliefs.iter().enumerate().map(|(i, b)| {
...
pub fn render_evolution(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let rows: Vec<Row> = app.data.evolution.iter().enumerate().map(|(i, e)| {
        let compliance_str = e.compliance_rate
...
pub fn render_pdca(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let rows: Vec<Row> = app.data.pdca.iter().enumerate().map(|(i, p)| {
        ...
        let experiment_str = if p.experiment.unwrap_or(false) { "yes" } else { "" };
...
pub fn render_conscience(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let rows: Vec<Row> = app.data.signals.iter().enumerate().map(|(i, s)| {
